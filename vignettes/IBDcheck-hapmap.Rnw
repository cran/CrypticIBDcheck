...
We leave the QC filtering options at their defaults. We specify 
...
The full panel of SNPs was thinned to an approximately independent
subset, from which IBD coefficients were estimated.
...
is eliminated by thinning the SNPs to an approximately 
